...
REM tools\3dstool -cvt0127f cci build\0.cxi build\1.cfa build\2.cfa build\7.cfa "Mario Tennis Open (TWN) (zh-Hant).3ds" --header cci\ncsdheader.bin
tools\3dstool -cvt0127f cci build\0.cxi cci\1.cfa cci\2.cfa cci\7.cfa "Mario Tennis Open (TWN) (zh-Hant).3ds" --header cci\ncsdheader.bin
IF NOT "%~1"=="" (
tools\3dstool --diff -vt cci --old "%~1" --new "Mario Tennis Open (TWN) (zh-Hant).3ds" --patch-file patch.3ps
tools\icn2ico cci\cxi0\exefs\icon.icn zh\icon.ico
CALL make_3ps_exe.bat zh\icon.ico patch.3ps zh-Hant_AGAWTOOLS || CALL ..\exepatch\make_3ps_exe.bat zh\icon.ico patch.3ps zh-Hant_AGAWTOOLS
DEL zh\icon.ico
)
POPD
